...
void Init_ADS1256_GPIO(void)
{
    GPIO_InitTypeDef GPIO_InitStructure;
        ...
        GPIO_InitStructure.GPIO_Pin = PIN_DRDY; 
          GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;//输入上拉接口
          GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
...
signed int ADS1256ReadData(unsigned char channel)  
{
    ...
    unsigned int sum=0;
        ...
        sum |= (SPI_WriteByte(0xff) << 16);//读取一个字节并左移16位，AD高24~17位
        sum |= (SPI_WriteByte(0xff) << 8);//读取一个字节并左移8位，AD中16~9位
        sum |= SPI_WriteByte(0xff);//读取一个字节，AD低8~1位
        CS_1();
